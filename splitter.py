...
class Splitter:
    ...
    def __hash__(self):
        return 1_000_000 * self.i1 + 10_000 * (
            self.i2 or 0
        ) + 100 * self.o1 + self.o2
...
class SplState:
    ...
    def genesis(cls, m: int) -> SplState:
        return cls(
            n_points=m,
            free_outputs=frozenset(range(m)),
            spls=frozenset(),
        )
    ...
    def spawn(self) -> ty.Generator[SplState, None, None]:
        ...
        for i in self.free_outputs - illegal_free_outputs:
            yield self._move_add_xi2o_spl(i)

        for i, j in combinations(self.free_outputs - illegal_free_outputs, 2):
        ...
        for i_spl in self.spls:
            ...
            for o_spl in self.spls:
                if o_spl.o1 in self.free_outputs:
                    yield self._move_add_backflow(i_spl, o_spl, 0)
                elif o_spl.o2 in self.free_outputs:
...
#
    ...
    def heurloss(self, out_target):
        return 20 * len(self.spls
                        ) + 5 * np.abs(len(self.free_outputs) - out_target)
