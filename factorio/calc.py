...
class BaseManuf:
    ...
    def with_modules(self, modules: t.List[Item.Module]) -> BaseManuf:
        if len(modules) > self.module_slots:
            raise ValueError(f"Too many modules for a {self.name} manufactory!")
        if (
            self.recipe
            and any(m.prod != Frac(0) for m in modules)
    ...
    def with_recipe(self, recipe: Recipe) -> BaseManuf:
        ...
        if (
            self.modules
            and any(m.prod != Frac(0) for m in self.modules)
